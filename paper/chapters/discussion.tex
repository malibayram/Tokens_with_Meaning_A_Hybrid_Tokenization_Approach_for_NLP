...
\textbf{Interpretation of TR-MTEB results.}

On the broader TR-MTEB suite, results are closer between tokenizers than on STS. This is expected because the benchmark spans heterogeneous task types (retrieval, classification, clustering, STS), and performance depends on factors beyond tokenization (training recipe, data distribution, and model architecture). Two takeaways are robust in our runs:
(i) TurkishTokenizer does not degrade performance across the suite and remains competitive, and
(ii) TurkishTokenizer yields the clearest improvements on semantic similarity, consistent with the hypothesis that morpheme-level clarity supports stable sentence representations.
